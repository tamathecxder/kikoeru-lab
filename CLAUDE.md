...
- **Dark mode**: `data-theme` attribute on `<html>`, set pre-paint from `localStorage['kikoeru-theme']` else `prefers-color-scheme`. Toggle is 11px lowercase text `light`/`dark` in the footer (`components/theme-toggle.tsx`) — never a sun/moon icon.
...
- **Filter bar**: one 11px lowercase row, items 26px apart; active = `border-bottom:1px solid var(--text)` + `padding-bottom:5px`. Not pills/buttons/dropdowns.
